  ...
  return (
    <div className="relative flex h-[100dvh] w-full flex-col bg-background-dark text-white overflow-hidden">
      <header className="sticky top-0 z-50 bg-background-dark/80 backdrop-blur-md border-b border-primary/10 p-4 flex items-center justify-between">
        ...
        <h1 className="text-lg font-bold tracking-tight">Order Details</h1>
      ...
      <main className="flex-1 overflow-y-auto pb-24 no-scrollbar">
          ...
          <div className="bg-primary/5 rounded-xl border border-primary/10 p-5">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-right">
                ...
                <span className="inline-flex items-center rounded-full bg-primary/20 px-2.5 py-0.5 text-xs font-medium text-primary">{order.status}</span>
              </div>
        ...
        <div className="px-4 mb-6">
          <div className="relative overflow-hidden rounded-xl border border-primary/20 bg-primary/10 backdrop-blur-md">
            <table className="w-full text-left text-sm">
              <thead className="bg-primary/20 text-primary/80 uppercase text-[10px] font-bold">
                <tr>
        ...
        <div className="px-4 grid grid-cols-2 gap-4">
          <button className="flex items-center justify-center gap-2 bg-primary text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/30 active:scale-95 transition-transform">
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-primary/10 bg-background-dark/80 backdrop-blur-lg px-4 pb-6 pt-2 z-20">
        <div className="flex justify-between items-center max-w-md mx-auto">
          ...
          <InvoiceNavItem icon={<ReceiptText className="size-5 fill-primary" />} label="Orders" active />
...
function InvoiceNavItem({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) {
  return (
    <div className={`flex flex-1 flex-col items-center justify-center gap-1 cursor-pointer ${active ? 'text-primary' : 'text-primary/40'}`} onClick={onClick}>
      {icon}
