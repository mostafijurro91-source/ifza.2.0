...
export default function AdminDashboard({ setScreen, onLogout }: { setScreen: (s: Screen) => void, onLogout: () => void }) {
  return (
    <div className="relative flex h-[100dvh] w-full flex-col bg-background-light dark:bg-[#0a0a0f] text-slate-900 dark:text-white overflow-hidden transition-colors duration-300">
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-[#0a0a0f]/80 backdrop-blur-md p-4 flex items-center justify-between border-b border-slate-200 dark:border-white/5">
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar p-5 space-y-8 pb-24">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl py-3.5 pl-10 pr-4 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
        ...
        <section className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-sm">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#0a0a0f] border-t border-slate-200 dark:border-white/5 pb-8 pt-3 px-6 flex justify-between items-center z-50 shadow-2xl">
...
function ActionCard({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
      className="flex-none w-28 aspect-square bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col items-center justify-center gap-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-white/10 transition-colors shadow-sm"
    >
...
function RecentOrder({ id, customer, time, price, status, statusColor, image }: { id: string, customer: string, time: string, price: string, status: string, statusColor: string, image: string }) {
  return (
    <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-4 flex items-center gap-4 shadow-sm">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-start">
          ...
          <p className="font-bold text-slate-900 dark:text-white">{price}</p>
        </div>
